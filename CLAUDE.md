...
**The hard problem (ROOT CAUSE FOUND 2026-06-05 - it was never anti-bot):** The fake "0 results" stores are sellers whose home eBay marketplace is not `ebay.com` (e.g. Australian sellers on `ebay.com.au`). eBay only serves a seller's full item grid on their home domain to an in-country viewer. We crawled every seller on `ebay.com` from US IPs, so non-US sellers genuinely returned "0 results". Proven live: both known-hard stores (`goodpricepeople`, `af2018`) return the full 240-item grid through the existing IPRoyal proxy on `ebay.com.au` with an AU exit IP, no challenge. The scraper's existing `apply_proxy_country` (`scraper/fetch.py`) already pins the exit IP to the domain's country, so the fix is domain routing. **No unblocker API needed for this class.** Full evidence: `docs/findings/2026-06-05-hard-stores-are-marketplace-mismatch.md`. Prior testing only ever varied the IP pool (IPRoyal vs Decodo), never the eBay domain, which is why "switching pools didn't fix it". Remaining build: detect each seller's home marketplace (probe a small ordered domain list on first 0-results, persist `(domain, country)` per store). n=2 known stores both Australian, so the fix must degrade gracefully when no probed marketplace returns items.

Earlier mitigations still in place: discovery is proxy-first then falls back to the coordinator's genuine IP, every store reported OK/0-results/blocked, failures saved to `~/.config/ebay-scraper/failed_stores.txt`, `scraper scrape retry` re-runs them. The separate single-IP reputation ceiling (one IP challenges after ~3,500 fast requests) is real and unrelated to the marketplace bug.

**Decisions pending for Fabien/Kieran:**
1. **Unblocker API? Likely NOT needed.** Tested Oxylabs eBay Scraper API trial 2026-06-05: it does NOT fix the hard stores either (`goodpricepeople` still 0 results on `ebay.com`), because the cause was domain, not blocking. The free domain-routing fix above is proven. Oxylabs is only worth revisiting if, after domain detection ships, a residual set of stores still fails for a genuine anti-bot reason. ~10 of 2000 trial requests used.
